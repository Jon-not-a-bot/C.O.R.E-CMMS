...
// Auto-assign tech based on category
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    let query = `SELECT wo.*, a.name as asset_name, a.category as asset_category, a.location as asset_location
      FROM work_orders wo LEFT JOIN assets a ON wo.asset_id = a.id WHERE 1=1`;
    const params = [];
    let idx = 1;
  ...
  try {
    const result = await db.query(
      `SELECT wo.*, a.name as asset_name, a.category as asset_category, a.location as asset_location
       FROM work_orders wo LEFT JOIN assets a ON wo.asset_id = a.id WHERE wo.id = $1`,
      [req.params.id]
    );
...
router.post('/', upload.array('photos', 5), async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(`
      INSERT INTO work_orders (wo_number, title, description, type, priority, status, asset_id, location, due_date, assigned_to, requester_name, source, photos, category)
      VALUES ($1,$2,$3,$4,$5,'Open',$6,$7,$8,$9,$10,$11,$12,$13) RETURNING *`,
      [wo_number, title, description, type || 'Repair', priority || 'Medium',
       asset_id || null, location, due_date || null, assigned_to,
       requester_name || 'CORE User', source || 'Internal', JSON.stringify(photos), category || 'General']
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(`
      UPDATE work_orders SET title=$1, description=$2, type=$3, priority=$4, status=$5,
      asset_id=$6, location=$7, due_date=$8, assigned_to=$9, resolution_notes=$10, category=$11,
      closed_at=${closed_at ? 'NOW()' : 'closed_at'}, updated_at=NOW()
      WHERE id=$12 RETURNING *`,
      [title, description, type, priority, status, asset_id || null, location,
       due_date || null, assigned_to, resolution_notes, category || 'General', req.params.id]
    );
    res.json(result.rows[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
