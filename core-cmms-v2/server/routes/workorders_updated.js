...
const CATEGORY_ASSIGNMENTS = {
  'PIT Fleet': 'PIT Tech', 'Dock Equipment': 'Dock Tech', 'HVAC & Heating': 'HVAC Tech',
  'Electrical': 'Electrical Tech', 'Life Safety': 'Safety Tech',
  'Building Envelope': 'Facilities Tech', 'Utilities': 'Facilities Tech', 'General': 'Facilities Tech'
};
...
router.post('/', upload.array('photos', 5), async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO work_orders (wo_number, title, description, type, priority, status, asset_id, location, due_date, assigned_to, requester_name, source, photos, category) VALUES ($1,$2,$3,$4,$5,'Open',$6,$7,$8,$9,$10,$11,$12,$13) RETURNING *`,
...
router.put('/:id', async (req, res) => {
  const db = req.app.locals.db;
  try {
    const { title, description, type, priority, status, asset_id, location, due_date, assigned_to, resolution_notes, category, checklist } = req.body;
    const result = await db.query(
      `UPDATE work_orders SET title=$1, description=$2, type=$3, priority=$4, status=$5, asset_id=$6, location=$7, due_date=$8, assigned_to=$9, resolution_notes=$10, category=$11, checklist=COALESCE($12, checklist), closed_at=${status === 'Closed' ? 'NOW()' : 'closed_at'}, updated_at=NOW() WHERE id=$13 RETURNING *`,
      [title, description, type, priority, status, asset_id || null, location, due_date || null, assigned_to, resolution_notes, category || 'General', checklist ? JSON.stringify(checklist) : null, req.params.id]
    );
    const wo = result.rows[0];

    // Auto-generate next PM WO when this one is closed
    if (status === 'Closed' && wo.pm_template_id) {
      try {
        const pmRouter = require('./pm-templates');
        const tmpl = await db.query('SELECT * FROM pm_templates WHERE id = $1', [wo.pm_template_id]);
        if (tmpl.rows.length > 0 && tmpl.rows[0].active) {
          await pmRouter.generateWOs(db, tmpl.rows[0]);
          console.log(`Auto-generated next PM WO for template ${wo.pm_template_id}`);
        }
      } catch (e) { console.error('PM auto-generate error:', e.message); }
    }
