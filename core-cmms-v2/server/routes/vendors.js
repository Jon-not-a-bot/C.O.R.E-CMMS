...
router.get('/:id', async (req, res) => {
  ...
  } catch (err) { res.status(500).json({ error: 'Failed to fetch vendor' }); }
});
...
router.post('/', async (req, res) => {
  ...
  // scope can be array or string
  const scopeVal = Array.isArray(scope) ? scope : (scope ? [scope] : []);
  try {
    const result = await pool.query(
      ...
      [name, scopeVal, phone || null, email || null, primary_contact || null, website || null, notes || null,
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE vendors SET name=$1, scope=$2, phone=$3, email=$4, primary_contact=$5,
       website=$6, notes=$7, contract_start=$8, contract_end=$9, contract_notes=$10, updated_at=NOW()
