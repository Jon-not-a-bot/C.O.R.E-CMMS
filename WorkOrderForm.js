...
export default function WorkOrderForm() {
  ...
  const [form, setForm] = useState({
    ...
    assigned_to:'Facilities Tech', resolution_notes:''
  });
  ...
  useEffect(() => {
    fetch(`${API}/api/assets`).then(r => r.json()).then(setAssets).catch(() => {});
    if (isEdit) {
      fetch(`${API}/api/workorders/${id}`).then(r => r.json()).then(wo => {
        setForm({
  ...
  const set = (k, v) => {
    setForm(f => {
      ...
      if (k === 'asset_id' && v) {
        ...
        if (asset) { next.category = asset.category; next.assigned_to = CATEGORY_TECH[asset.category] || 'Facilities Tech'; next.location = asset.location || next.location; }
      }
      return next;
    });
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      const body = isEdit ? JSON.stringify(form) : data;
      const headers = isEdit ? { 'Content-Type': 'application/json' } : {};
